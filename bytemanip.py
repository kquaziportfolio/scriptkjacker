    ...
    if opcode in dis.hasconst:
        if constants is not None:
            try:
                ...
                try:
                    oparg = constants.index(
                        argval.encode().decode("unicode-escape")[1:-1]
                    )
                except:
...
def assemble(code_list, c):
    constants, varnames, names, cell_names = (
        c.co_const,
        c.co_varnames,
        c.co_names,
    ...
    for i, instruction in enumerate(code_list):
        if len(instruction) == 2:
            ...
            oparg = get_oparg(
                i * 2, opcode, argval, constants, varnames, names, cell_names
            )
        else:
